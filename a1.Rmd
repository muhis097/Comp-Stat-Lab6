...
In below graphs black dots represent values of initial population, green dots represent values of population after final iteration, and the red dot represent max value after final iteration.
...
from above graphs we can see that genetic algorithm fails to find global maximum value when number of iterations are 10 even when we set mutation probability to 0.9, but when we increase number of iterations we see that the population moves toward global maximum value. 
